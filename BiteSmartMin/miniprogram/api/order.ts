...
export interface Order { id: number | string; orderNo?: string; orderStatus?: number; totalAmount?: number; deliveryType?: number; deliveryFee?: number; receiverName?: string; receiverPhone?: string; deliveryAddress?: string; createTime?: string; refundId?: number | string }
...
export const createOrder = (data: { address: string; receiverName: string; receiverPhone: string; remark?: string; deliveryType?: number; latitude?: number; longitude?: number }): Promise<{ orderNo: string }> => request<{ orderNo: string }>({ url: '/orders', method: 'POST', data, contentType: 'form' })
