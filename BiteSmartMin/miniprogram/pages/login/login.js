...
Component({
    ...
    lifetimes: {
        attached() {
            const user = (0, auth_2.getUserInfo)();
            if ((0, auth_2.getToken)() && user && [10, 20, 30].includes(Number(user.roleType))) {
    ...
    methods: {
        ...
        login() {
            ...
            if (this.data.mode === 'account') {
                (0, auth_1.loginWithAccount)(this.data.username, this.data.password, this.data.roleType)
                    .then(() => this.enterApp('/pages/index/index'))
                    .catch((error) => this.setData({ errorMessage: error.message || 'Login failed' }))
                    .finally(() => this.setData({ loading: false }));
                return;
            }
            wx.login({
                success: (result) => {
                    (0, auth_1.loginWithWechat)(result.code, this.data.roleType)
                        .then((response) => {
