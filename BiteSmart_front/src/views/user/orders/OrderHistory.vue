...
<template>
  <div class="orders-page">
    ...
    <div v-loading="loading" class="order-list">
        ...
        <div class="order-card-head"><div class="order-meta"><span class="status-dot" :class="statusTone(order)"></span><strong>{{ orderStatus(order) }}</strong><span>{{ formatDate(order.createTime) }}</span></div><span class="order-number">{{ orderLabel(order.orderNo) }}</span></div>
    ...
    <el-dialog v-model="detailVisible" class="order-detail-dialog" width="700px" :show-close="true">
      <template #header><div class="detail-dialog-head"><div><span class="eyebrow">订单详情</span><h2>{{ detail ? orderLabel(detail.orderNo) : '' }}</h2></div><span class="detail-status" :class="statusTone(detail || {})">{{ detail ? orderStatus(detail) : '' }}</span></div></template>
...
<style scoped>
.orders-page{max-width:1050px;margin:0 auto;padding:66px 24px 90px;color:#1f2a24}.orders-intro{display:flex;align-items:flex-end;justify-content:space-between;margin-bottom:38px}.eyebrow{display:block;color:var(--orange);font-size:11px;font-weight:700;letter-spacing:.14em}.orders-intro h1{margin:12px 0 8px;font-family:"Source Han Serif SC","Noto Serif CJK SC","Songti SC",serif;font-size:38px;font-weight:600;line-height:1.2}.orders-intro p{margin:0;color:#718078;font-size:13px}.order-count{display:flex;align-items:baseline;gap:8px;padding-bottom:4px;color:#8a968e}.order-count strong{color:var(--green);font-family:Georgia,serif;font-size:32px;font-weight:500}.order-count span{font-size:12px}.order-list{min-height:260px}.order-card{margin-bottom:18px;border:1px solid #e1e9e2;background:#fff;transition:border-color .2s,box-shadow .2s}.order-card:hover{border-color:#bfd2c2;box-shadow:0 8px 22px rgba(30,158,98,.06)}.order-card-head,.order-card-foot{display:flex;align-items:center;justify-content:space-between;gap:18px;padding:14px 18px}.order-card-head{border-bottom:1px solid #edf1ec}.order-meta{display:flex;align-items:center;gap:9px;font-size:12px}.order-meta strong{font-size:13px}.order-meta>span:last-child{color:#8a968e}.status-dot{width:7px;height:7px;border-radius:50%;background:#9da8a0}.status-dot.pending{background:var(--orange)}.status-dot.processing{background:var(--green)}.status-dot.shipping{background:var(--green-deep)}.status-dot.completed{background:var(--green-ink)}.order-number{color:#9aa49d;font-size:11px}.order-card-body{display:flex;align-items:center;justify-content:space-between;gap:30px;padding:22px 18px}.order-summary{display:flex;align-items:center;min-width:0;gap:13px}.summary-mark{display:grid;place-items:center;width:42px;height:42px;flex:0 0 42px;background:#f2f7f2;color:var(--green)}.summary-mark svg{width:18px}.order-summary strong{display:block;overflow:hidden;color:#1f2a24;font-size:14px;text-overflow:ellipsis;white-space:nowrap}.order-summary p{max-width:600px;margin:7px 0 0;overflow:hidden;color:#89958d;font-size:11px;text-overflow:ellipsis;white-space:nowrap}.order-amount{display:flex;flex-direction:column;align-items:flex-end;flex:0 0 auto;gap:5px}.order-amount span{color:#89958d;font-size:11px}.order-amount strong{color:var(--green);font-size:21px;font-weight:650}.order-card-foot{border-top:1px solid #edf1ec}.order-hint{color:#a0aaa3;font-size:11px}.order-actions{display:flex;align-items:center;gap:4px}.order-actions :deep(.el-button){margin:0;font-size:12px}.order-actions :deep(svg){width:14px;margin-right:4px;vertical-align:-2px}.order-empty{display:flex;flex-direction:column;align-items:center;justify-content:center;min-height:340px;background:#f1f5f1;text-align:center}.empty-mark{display:grid;place-items:center;width:54px;height:54px;margin-bottom:17px;background:#dfece1;color:var(--green)}.empty-mark svg{width:24px}.order-empty h2{margin:0 0 8px;font-family:"Source Han Serif SC","Noto Serif CJK SC","Songti SC",serif;font-size:20px}.order-empty p{margin:0 0 18px;color:#78857d;font-size:12px}.order-empty :deep(.el-button svg){width:14px;margin-left:5px;vertical-align:-2px}.pager{display:flex;justify-content:center;margin-top:32px}.order-detail-dialog :deep(.el-dialog__header){margin:0;padding:24px 28px 18px;border-bottom:1px solid #edf1ec}.order-detail-dialog :deep(.el-dialog__body){padding:20px 28px 24px}.detail-dialog-head{display:flex;align-items:flex-end;justify-content:space-between;gap:15px}.detail-dialog-head h2{margin:9px 0 0;color:#1f2a24;font-size:20px}.detail-status{padding:5px 9px;background:#f2f7f2;color:var(--green);font-size:12px}.detail-status.pending{background:#fff4ed;color:#b55e43}.detail-status.shipping{background:#fff8ea;color:#aa762e}.detail-status.muted{background:#f0f3f0;color:#77847b}.detail-section{margin-bottom:22px}.detail-section-title{display:flex;align-items:baseline;justify-content:space-between;margin-bottom:10px;color:#1f2a24;font-size:14px;font-weight:700}.detail-section-title small{color:#8a968e;font-size:11px;font-weight:400}.detail-items{border-top:1px solid #e1e9e2}.detail-item{display:flex;align-items:center;gap:11px;padding:11px 0;border-bottom:1px solid #edf1ec}.item-image{display:grid;place-items:center;width:40px;height:40px;flex:0 0 40px;background:#eff4ef;color:#74907a;font-size:12px}.item-image img{width:100%;height:100%;object-fit:cover}.item-copy{display:flex;flex:1;flex-direction:column;gap:5px}.item-copy strong{color:#1f2a24;font-size:13px}.item-copy span{color:#8a968e;font-size:11px}.item-price{color:var(--green);font-size:13px}.detail-empty{margin:0;padding:14px 0;color:#8a968e;font-size:12px}.delivery-info{padding:14px 15px;background:#f7faf7}.delivery-info div{display:flex;align-items:center;gap:8px;color:#1f2a24;font-size:13px}.delivery-info svg{width:16px;color:var(--green)}.delivery-info p{margin:9px 0 0 24px;color:#78857d;font-size:12px;line-height:1.6}.detail-total{display:flex;align-items:center;justify-content:space-between;padding:16px 0;border-top:1px solid #e1e9e2;border-bottom:1px solid #e1e9e2}.detail-total span{color:#78857d;font-size:12px}.detail-total strong{color:var(--green);font-size:22px}.detail-note{display:grid;grid-template-columns:auto 1fr auto 1fr;gap:8px 13px;margin-top:15px;color:#8a968e;font-size:11px}.detail-note strong{color:#4e5e54;font-weight:500;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}@media(max-width:650px){.orders-page{padding:44px 18px 68px}.orders-intro{align-items:flex-start;flex-direction:column;gap:20px}.orders-intro h1{font-size:31px}.order-card-head,.order-card-foot{align-items:flex-start;flex-direction:column;gap:10px}.order-card-body{align-items:flex-start;flex-direction:column;gap:18px}.order-summary{width:100%}.order-summary p{max-width:calc(100vw - 110px)}.order-amount{align-items:flex-start}.order-actions{flex-wrap:wrap}.order-detail-dialog{width:calc(100% - 28px)!important;margin-top:8vh!important}.order-detail-dialog :deep(.el-dialog__header),.order-detail-dialog :deep(.el-dialog__body){padding-left:20px;padding-right:20px}.detail-note{grid-template-columns:auto 1fr}.detail-note span:nth-of-type(2){grid-column:1}.detail-note strong:nth-of-type(2){grid-column:2}}
</style>
<style scoped>
