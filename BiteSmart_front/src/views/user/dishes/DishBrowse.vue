...
const dishImageUrl = (dish: any) => resolveFileUrl(dish?.dishImage) || ''
// tags / fitScenes / cautions 是后端 JSON 数组字符串列，统一走 parseJsonList 防御解析
const dishTags = (dish: any) => parseJsonList(dish?.tags).slice(0, 2)
const detailTags = computed(() => parseJsonList(detailData.value?.tags))
const detailFitScenes = computed(() => parseJsonList(detailData.value?.fitScenes))
const detailCautions = computed(() => parseJsonList(detailData.value?.cautions))
  ...
  <div class="dish-page">
    ...
    <div v-loading="loading" class="dish-grid">
      <article v-for="dish in dishes" :key="dish.id" class="dish-card">
        <button class="dish-image" @click="handleViewDetail(dish)"><img v-if="dishImageUrl(dish)" :src="dishImageUrl(dish)" :alt="dish.dishName" /><span v-else class="img-placeholder">{{ (dish.dishName || '菜')[0] }}</span><span>{{ dish.calories || 0 }} kcal</span></button>
        <div class="dish-copy"><div class="dish-top"><h2>{{ dish.dishName }}</h2><strong>¥{{ formatPrice(dish.price) }}</strong></div><p>{{ dish.description || '新鲜食材搭配，轻松吃得更均衡。' }}</p><div v-if="dishTags(dish).length" class="dish-tags"><span v-for="tag in dishTags(dish)" :key="tag">{{ tag }}</span></div><div class="dish-bottom"><small>蛋白质 {{ dish.protein || 0 }}g</small><button class="add-button" :class="{ added: addedDishId === dish.id }" :disabled="addingDishId === dish.id" :aria-label="addedDishId === dish.id ? '已加入购物车' : '加入购物车'" @click.stop="handleAddToCart(dish)"><Check v-if="addedDishId === dish.id" /><Plus v-else /><span>{{ addedDishId === dish.id ? '已加入' : '加入' }}</span></button></div></div>
      </article>
  ...
  <el-dialog v-model="detailVisible" class="dish-detail-dialog" width="680px" :show-close="true">
    <template v-if="detailData">
      <div class="dish-detail-hero">
        ...
        <div v-else class="detail-cover-placeholder">{{ (detailData.dishName || '菜')[0] }}</div>
        <div class="dish-detail-info">
          <span class="detail-eyebrow">精选菜品</span>
          <h2>{{ detailData.dishName }}</h2>
          <p>{{ detailData.description || '新鲜食材搭配，轻松吃得更均衡。' }}</p>
          <strong class="detail-price">¥{{ formatPrice(detailData.price) }}</strong>
        </div>
      </div>
      <div v-if="detailTags.length" class="detail-tags"><span v-for="tag in detailTags" :key="tag">{{ tag }}</span></div>
      ...
      <div v-if="detailData.aiComment" class="ai-quote">
        <div class="quote-head"><el-icon :size="15"><MagicStick /></el-icon><span>AI 营养师点评</span></div>
        <p>{{ detailData.aiComment }}</p>
      </div>
      <div v-if="detailFitScenes.length || detailCautions.length" class="fit-caution">
        <div v-if="detailFitScenes.length" class="fit-col">
          <div class="fit-head ok"><span class="mk">✓</span>适宜人群</div>
          <div v-for="scene in detailFitScenes" :key="scene" class="fit-item">{{ scene }}</div>
        </div>
        <div v-if="detailCautions.length" class="fit-col">
          <div class="fit-head no"><span class="mk">×</span>注意事项</div>
          <div v-for="caution in detailCautions" :key="caution" class="fit-item">{{ caution }}</div>
        </div>
      </div>
    </template>
    <template #footer>
...
<style scoped>
.dish-page{max-width:1192px;margin:0 auto;padding:66px 24px 90px;color:#1f2a24}.browse-intro{margin-bottom:38px}.browse-intro span{display:block;color:var(--orange);font-size:12px;font-weight:650;letter-spacing:.14em}.browse-intro h1{margin:12px 0 8px;font-family:"Source Han Serif SC","Noto Serif CJK SC","Songti SC",serif;font-size:38px;font-weight:600}.browse-intro p{margin:0;color:#6c786f;font-size:14px}.dish-grid{display:grid;grid-template-columns:repeat(3,1fr);gap:30px 22px;min-height:300px}.dish-card{background:#fff;border-bottom:1px solid #dfe7df}.dish-image{display:block;position:relative;width:100%;height:230px;padding:0;border:0;background:#eef2ed;overflow:hidden;cursor:pointer}.dish-image img{width:100%;height:100%;object-fit:cover;transition:transform .35s ease}.dish-card:hover .dish-image img{transform:scale(1.04)}.dish-image span{position:absolute;left:13px;bottom:12px;padding:5px 8px;background:#fff;color:var(--green);font-size:11px}.dish-copy{padding:16px 2px 15px}.dish-top{display:flex;justify-content:space-between;gap:12px}.dish-top h2{margin:0;font-size:18px;font-weight:650}.dish-top strong{color:var(--green);font-size:17px}.dish-copy p{min-height:38px;margin:9px 0;color:#718078;font-size:12px;line-height:1.6}.dish-bottom{display:flex;align-items:center;justify-content:space-between;padding-top:12px;border-top:1px solid #edf1ec}.dish-bottom small{color:#78857d;font-size:11px}.dish-bottom button{border:0;background:none;color:var(--green);font-weight:650;cursor:pointer}.dish-bottom button span{margin-left:5px;color:var(--orange);font-size:18px;vertical-align:-1px}.dish-empty{grid-column:1/-1;display:flex;align-items:center;justify-content:center;gap:26px;min-height:280px;background:#eff3ee}.dish-empty img{width:145px;height:145px;object-fit:cover}.dish-empty h2{margin:0 0 8px;font-family:"Source Han Serif SC","Songti SC",serif}.dish-empty p{margin:0;color:#718078;font-size:13px}.pager{display:flex;justify-content:center;margin-top:46px}.dish-detail{display:flex;gap:18px}.dish-detail .dish-image{width:150px;height:120px;flex-shrink:0;cursor:default}.dish-detail h3{margin:0 0 10px}.dish-detail p{color:#6c786f;line-height:1.6}.dish-detail strong{font-size:20px;color:var(--green)}@media(max-width:800px){.dish-page{padding:45px 18px 68px}.dish-grid{grid-template-columns:repeat(2,1fr)}.dish-image{height:210px}}@media(max-width:540px){.browse-intro h1{font-size:31px}.dish-grid{grid-template-columns:1fr}.dish-image{height:250px}}
.add-button{display:inline-flex;align-items:center;justify-content:center;gap:6px;min-width:76px;height:32px;padding:0 12px;border:1px solid #c9dbce;border-radius:4px;background:#f5f8f4;color:var(--green);font-size:12px;font-weight:700;line-height:1;cursor:pointer;transition:background .2s ease,border-color .2s ease,color .2s ease,transform .2s ease}.add-button svg{width:15px;height:15px}.add-button span{margin-left:0;color:inherit;font-size:inherit;vertical-align:initial}.add-button:hover:not(:disabled){border-color:var(--green);background:var(--green);color:#fff;transform:translateY(-1px)}.add-button:active:not(:disabled){transform:translateY(0)}.add-button:disabled{cursor:wait;opacity:.65}.add-button.added{border-color:var(--green);background:var(--green);color:#fff}
.dish-detail-dialog :deep(.el-dialog__header){margin:0;padding:24px 28px 16px;border-bottom:1px solid #edf1ec}.dish-detail-dialog :deep(.el-dialog__title){color:#1f2a24;font-size:18px;font-weight:700}.dish-detail-dialog :deep(.el-dialog__headerbtn){top:20px;right:22px}.dish-detail-dialog :deep(.el-dialog__body){padding:24px 28px 20px}.dish-detail-dialog :deep(.el-dialog__footer){padding:16px 28px 22px;border-top:1px solid #edf1ec}.dish-detail-hero{display:grid;grid-template-columns:230px 1fr;gap:26px;align-items:center}.dish-detail-cover{width:230px;height:190px;background:#eef2ed}.dish-detail-info{min-width:0}.detail-eyebrow{display:block;margin-bottom:10px;color:var(--orange);font-size:11px;font-weight:700;letter-spacing:.12em}.dish-detail-info h2{margin:0;color:#1f2a24;font-family:"Source Han Serif SC","Noto Serif CJK SC","Songti SC",serif;font-size:26px;line-height:1.25}.dish-detail-info p{margin:13px 0 18px;color:#718078;font-size:13px;line-height:1.8}.detail-price{color:var(--green);font-size:27px;letter-spacing:.02em}.nutrition-heading{display:flex;align-items:baseline;justify-content:space-between;margin:26px 0 10px}.nutrition-heading span{color:#1f2a24;font-size:14px;font-weight:700}.nutrition-heading small{color:#8a968e;font-size:11px}.nutrition-grid{display:grid;grid-template-columns:repeat(4,1fr);border:1px solid #e1e9e2;background:#f8faf8}.nutrition-grid>div{display:flex;flex-direction:column;gap:8px;padding:15px 14px;border-right:1px solid #e1e9e2}.nutrition-grid>div:last-child{border-right:0}.nutrition-grid span{color:#78857d;font-size:11px}.nutrition-grid strong{color:var(--green);font-size:18px}.nutrition-grid strong small{margin-left:3px;color:#8a968e;font-size:10px;font-weight:500}.detail-footer{display:flex;align-items:center;justify-content:space-between;gap:20px}.detail-footer>span{color:#78857d;font-size:12px}.detail-footer .el-button--primary{min-width:116px}@media(max-width:600px){.dish-detail-dialog{width:calc(100% - 28px)!important;margin-top:8vh!important}.dish-detail-dialog :deep(.el-dialog__header){padding:20px 20px 14px}.dish-detail-dialog :deep(.el-dialog__body){padding:18px 20px}.dish-detail-dialog :deep(.el-dialog__footer){padding:14px 20px 18px}.dish-detail-hero{grid-template-columns:1fr;gap:18px}.dish-detail-cover{width:100%;height:190px}.dish-detail-info h2{font-size:23px}.nutrition-grid{grid-template-columns:repeat(2,1fr)}.nutrition-grid>div:nth-child(2){border-right:0}.nutrition-grid>div:nth-child(-n+2){border-bottom:1px solid #e1e9e2}.detail-footer{align-items:stretch;flex-direction:column;gap:12px}.detail-footer>span{display:none}.detail-footer div{display:flex;justify-content:flex-end}.detail-footer .el-button--primary{flex:1}}
.browse-filters{display:flex;align-items:center;gap:10px;margin:-12px 0 30px}.browse-filters .filter-search{max-width:330px}.browse-filters :deep(.el-select){width:150px}@media(max-width:600px){.browse-filters{align-items:stretch;flex-wrap:wrap;margin:-8px 0 24px}.browse-filters .filter-search{max-width:none;width:100%}.browse-filters :deep(.el-select){flex:1;width:auto}.browse-filters .el-button{width:80px}}
</style>
<style scoped>
.dish-image .img-placeholder{display:flex;align-items:center;justify-content:center;width:100%;height:100%;position:static;padding:0;background:var(--green-soft);color:var(--green-deep);font-family:"Source Han Serif SC","Songti SC",serif;font-size:44px;font-weight:600}
.dish-tags{display:flex;flex-wrap:wrap;gap:6px;margin-bottom:10px}
.dish-tags span{padding:3px 8px;background:var(--green-soft);color:var(--green-deep);font-size:11px;border-radius:4px}
.detail-tags{display:flex;flex-wrap:wrap;gap:8px;margin-top:18px}
.detail-tags span{padding:4px 10px;background:var(--green-soft);color:var(--green-deep);font-size:12px;border-radius:4px}
.detail-cover-placeholder{display:flex;align-items:center;justify-content:center;width:230px;height:190px;background:var(--green-soft);color:var(--green-deep);font-family:"Source Han Serif SC","Songti SC",serif;font-size:52px}
.ai-quote{margin-top:24px;padding:4px 0 4px 18px;border-left:3px solid var(--green)}
.quote-head{display:flex;align-items:center;gap:8px;color:var(--green-deep);font-size:13px;font-weight:700;letter-spacing:.06em}
.ai-quote p{margin:10px 0 0;color:#718078;font-size:13px;line-height:1.9}
.fit-caution{display:grid;grid-template-columns:1fr 1fr;gap:28px;margin-top:24px}
.fit-head{display:flex;align-items:center;gap:7px;margin-bottom:6px;font-size:13px;font-weight:700}
.fit-head.ok{color:var(--green-deep)}
.fit-head.no{color:var(--danger-brand)}
.fit-head .mk{font-size:14px}
.fit-item{padding:9px 0;border-top:1px dashed var(--hair);color:#718078;font-size:12px;line-height:1.6}
@media(max-width:600px){.fit-caution{grid-template-columns:1fr}}
</style>
