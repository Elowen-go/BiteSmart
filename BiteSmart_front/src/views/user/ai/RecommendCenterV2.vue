...
const fallbackImage = '/images/home/meal-card.jpg'
const candidateOptions = (currentId: number) => candidates.value.filter((item: any) => item.dishId !== currentId)
const imageFor = (item: any) => resolveFileUrl(item.dishImage) || fallbackImage
...
const addRecipeToCart = async () => {
  ...
  try {
    for (const item of items) {
      ...
      merchants.add(Number(item.merchantId))
      success += 1
    }
...
<style scoped>
.summary-total strong,.target-list strong{color:#1f4d3a!important}
</style>
<style scoped>
...
.summary-heading .el-button{background:#1f4d3a;border-color:#1f4d3a;color:#fff}
.summary-heading .el-button:hover{background:#2c654e;border-color:#2c654e}
