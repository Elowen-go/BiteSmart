...
const buy = async (plan: any) => {
  try {
    await ElMessageBox.confirm(`确认购买${plan.planName}吗？价格 ¥${plan.price}`, '购买确认')
...
<template>
  <div class="membership-page">
    ...
    <section class="plans-section">
      <div class="section-heading"><div><span class="section-kicker">CHOOSE YOUR PLAN</span><h2>选择适合你的会员方案</h2></div><span class="heading-note">按需选择，随时查看有效期</span></div>
      <div v-loading="loading" class="plans-grid">
        <article v-for="(plan, index) in plans" :key="plan.id" class="plan-card" :class="{ featured: index === 1 }">
          ...
          <el-button class="plan-button" type="primary" :disabled="hasActiveMembership" @click="buy(plan)">{{ hasActiveMembership ? '当前已有有效会员' : '立即开通' }}</el-button>
        </article>
      </div>
...
<style scoped>
.membership-page{max-width:1180px;margin:0 auto;padding:42px 24px 80px;color:#253129}.membership-hero{display:flex;align-items:flex-end;justify-content:space-between;padding:8px 0 28px;border-bottom:1px solid #dfe7df}.eyebrow,.section-kicker{color:#cf704f;font-size:10px;font-weight:800;letter-spacing:.16em}.membership-hero h1{margin:9px 0 7px;font-family:"Source Han Serif SC","Songti SC",serif;font-size:32px;line-height:1.1}.membership-hero p{margin:0;color:#718078;font-size:13px}.active-membership{display:flex;align-items:center;justify-content:space-between;gap:24px;margin-top:24px;padding:24px 28px;background:#1f4d3a;color:#fff}.active-membership .section-kicker{color:#e6b08e}.active-membership h2{margin:10px 0 6px;font-family:"Source Han Serif SC","Songti SC",serif;font-size:26px}.active-membership p{margin:0;color:#d7e6d9;font-size:12px}.plans-section,.history-section{margin-top:24px;padding:24px 28px;background:#fff;border:1px solid #e1e9e2}.section-heading{display:flex;align-items:flex-end;justify-content:space-between;gap:20px;margin-bottom:22px}.section-heading h2{margin:7px 0 0;font-family:"Source Han Serif SC","Songti SC",serif;font-size:22px}.heading-note{color:#8a968e;font-size:11px}.plans-grid{display:grid;grid-template-columns:repeat(3,1fr);gap:16px}.plan-card{position:relative;display:flex;flex-direction:column;min-height:360px;padding:23px 22px;border:1px solid #dfe7df;background:#fbfdfb}.plan-card.featured{border:2px solid #1f4d3a;padding:22px 21px}.featured-label{position:absolute;top:-1px;right:20px;padding:5px 10px;background:#1f4d3a;color:#fff;font-size:10px}.plan-type{color:#cf704f;font-size:10px;font-weight:800;letter-spacing:.12em}.plan-card h3{margin:12px 0 0;font-family:"Source Han Serif SC","Songti SC",serif;font-size:20px}.plan-price{margin-top:22px;color:#1f4d3a;font-family:Georgia,serif;font-size:38px;font-weight:700}.plan-price small{margin-right:4px;font-family:inherit;font-size:16px}.plan-duration{margin:4px 0 18px;color:#8a968e;font-size:12px}.plan-card ul{display:grid;gap:10px;margin:0 0 22px;padding:16px 0;border-top:1px solid #edf1ec;list-style:none;color:#56655c;font-size:12px}.plan-card li::before{content:'✓';margin-right:8px;color:#cf704f;font-weight:700}.plan-button{width:100%;margin-top:auto;background:#1f4d3a;border-color:#1f4d3a}.plan-button:hover{background:#2c654e;border-color:#2c654e}.history-table{--el-table-border-color:#e5ece6;--el-table-header-bg-color:#f4f8f3}.history-table :deep(th.el-table__cell){height:46px;background:#f4f8f3;color:#718078;font-size:12px}.history-table :deep(td.el-table__cell){height:56px;color:#253129;font-size:13px}@media(max-width:800px){.plans-grid{grid-template-columns:1fr 1fr}.membership-hero{align-items:flex-start;gap:20px;flex-direction:column}}@media(max-width:600px){.membership-page{padding:30px 16px 58px}.active-membership{align-items:flex-start;flex-direction:column}.plans-section,.history-section{padding:18px}.plans-grid{grid-template-columns:1fr}.section-heading{align-items:flex-start;flex-direction:column;gap:8px}.history-table{font-size:12px}}
</style>
