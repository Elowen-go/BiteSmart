...
import { getComboList, addCombo, updateCombo, deleteCombo } from '../../../api/merchant/combos'
import type { Combo } from '../../../api/merchant/combos'
...
const handleAdd = () => {
  ...
  form.value = { name: '', price: 0, status: 1, description: '', imageUrl: '', originalPrice: 0, calories: 0 }
...
const handleSubmit = async () => {
  ...
  try {
    let res: any
    if (editingId.value) {
      res = await updateCombo(editingId.value, form.value as any)
    } else {
      res = await addCombo(form.value as any)
    }
    if (res.code === 200) {
